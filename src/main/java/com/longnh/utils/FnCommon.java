...
public class FnCommon {
  ...
  public static List<Long> stringToListLong(String ids) {
    if (!StringUtils.hasLength(ids)) return new ArrayList<>();
    try {
      return (List<Long>)
          Arrays.<String>stream(ids.split(",")).map(Long::new).collect(Collectors.toList());
    } catch (Exception e) {
      throw new ExceptionHandle(HttpStatus.BAD_REQUEST, "param ids invalid");
    }
  }

  public static List<Long> stringToListLongOrNull(String ids) {
    if (!StringUtils.hasLength(ids)) return null;
    try {
      return (List<Long>)
          Arrays.<String>stream(ids.split(",")).map(Long::new).collect(Collectors.toList());
    } catch (Exception e) {
